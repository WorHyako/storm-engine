...
bool SEA::Init()
{
    rs = static_cast<RENDER *>(core.GetService("render"));
    ...
    rs->CreateTexture(XWIDTH, YWIDTH, MIPSLVLS, {.isRenderTarget = true}, RHI::Format::RGBA8_UNORM, RHI::MemoryPropertiesBits::DEVICE_LOCAL_BIT,
    ...
    rs->CreateCubeTexture(128, 1, {.isRenderTarget = true}, RHI::Format::B5G6R5_UNORM, RHI::MemoryPropertiesBits::DEVICE_LOCAL_BIT, pEnvMap);
    rs->CreateCubeTexture(128, 1, { .isRenderTarget = true }, RHI::Format::B5G6R5_UNORM, RHI::MemoryPropertiesBits::DEVICE_LOCAL_BIT, pSunRoadMap);
    rs->CreateDepthStencilSurface(128, 128, RHI::Format::D24S8, 1/*D3DMULTISAMPLE_NONE*/, pZStencil);

    rs->CreateTexture(128, 128, 1, { .isRenderTarget = true }, RHI::Format::B5G6R5_UNORM, RHI::MemoryPropertiesBits::DEVICE_LOCAL_BIT, pReflection);
    rs->CreateTexture(128, 128, 1, { .isRenderTarget = true }, RHI::Format::B5G6R5_UNORM, RHI::MemoryPropertiesBits::DEVICE_LOCAL_BIT, pReflectionSunroad);
    rs->CreateDepthStencilSurface(128, 128, RHI::Format::D24S8, 1/*D3DMULTISAMPLE_NONE*/, pReflectionSurfaceDepth);
...
#define GET_MASSIVE(dx, dy)                                                                                            \
    (float(pMassive[((x + dx) & (XWIDTH - 1)) + ((y + dy) & (XWIDTH - 1)) * XWIDTH] & 0xFF) / 255.0f)
        ...
        if (!pVolumeTexture)
        {
            ...
            hr = rs->CreateTexture(XWIDTH, YWIDTH, MIPSLVLS, 0, RHI::Format::RGBA8_UNORM,
                RHI::MemoryPropertiesBits::HOST_VISIBLE_BIT | RHI::MemoryPropertiesBits::HOST_CACHED_BIT, pBumpMap);
...
void SEA::Realize(uint32_t dwDeltaTime)
{
    ...
    rs->GetTransform(TSType::TS_VIEW, &mView);
    mIView = mView;
    ...
    */
    ...
    if (bUnderSea && bUnderSeaEnable)
    {
        ...
        if (aTrashRects.size())
        {
            rs->TextureSet(iSeaTrashTexture, 0);
        ...
        {
            rs->TextureSet(iSeaLightTexture, 0, , );
