...
FONT::FONT(RENDER& renderer, RHI::DeviceHandle device)
...
int32_t FONT::UpdateVertexBuffer(int32_t x, int32_t y, const char* data_PTR, int utf8length, float scale, uint32_t color)
{
    ...
    FONT_CHAR_VERTEX* pVertex = (FONT_CHAR_VERTEX*)device_->mapBufferMemory(vertexBuffer_.get(), 0, sizeof(FONT_CHAR_VERTEX) * utf8length * SYM_VERTEXS);
